...
|---|---|---|---|
| PHP 8.4 legacy HTTP request-context crash | `tests/integration/checkout-request-context-http.sh` (server restart per matrix) + Compatibility cell `WP 7.1 / WC 11.1.0 / PHP 8.4 / legacy` | targeted 3-pass + full matrix (PR CI) | VERIFIED when CI green |
...
| AS cross-version WC 10.8/11.0/11.1 × legacy/HPOS | Compatibility matrix + AS runtime test | one cell per band | VERIFIED when matrix green |
...
| All eligible parents reached | `r6-large-store-benchmark.php` | cursor completes, no stuck window | VERIFIED |
| Synthetic 10k legacy/HPOS | `.github/workflows/r6-large-store-benchmark.yml` | 100/1k/5k/10k × 2 | VERIFIED when workflow green |
...
| Classic/Blocks checkout browser | `tests/e2e/r6-browser-ux.spec.ts` (Playwright) + default theme | screenshots + axe | VERIFIED when e2e job green |
| Mobile/desktop viewport | same | two viewports | VERIFIED |
| Console / broken assets / labels | same | assertions in spec | VERIFIED |
| Arabic/RTL default theme | `tests/e2e/r6-browser-ux.spec.ts` RTL project + locale ar | dir=rtl + checkout | VERIFIED |
| Local reverse proxy | `tests/integration/lib/reverse-proxy-smoke.sh` | callback URL, no-cache, spoofed headers fail-closed | VERIFIED |
| Real Cloudflare/CDN | — | edge product | EXTERNAL REQUIRED |
| Automated DAST | `tests/security/r6-dast-smoke.php` | authenticated/unauth probes | VERIFIED / findings closed |
| Professional pentest | — | independent | EXTERNAL REQUIRED |
| Secrets current tree | `tests/security/r6-secret-scan.sh` + CI | no real secrets | VERIFIED |
| Secrets git history / ZIP | same | history + package scan | VERIFIED |
| Actions pinned by SHA | `tests/security/r6-actions-pin-audit.sh` | all `uses:` pinned | VERIFIED |
